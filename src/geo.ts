...
export function computeDistanceBetween(
    ...
    const sinHalfDeltaLon = Math.sin((Math.PI * (coordinateTo.lon - coordinateFrom.lon)) / 360);
    const sinHalfDeltaLat = Math.sin((Math.PI * (coordinateTo.lat - coordinateFrom.lat)) / 360);
    const a =
